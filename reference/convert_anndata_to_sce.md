...
\`adata\$X\` (or \`adata\$layers\[\[counts_layer\]\]\` if present)
becomes the \`counts\` assay, additional layers become other assays,
\`adata\$obs\` is written to \`colData\`, \`adata\$var\` to \`rowData\`,
and \`adata\$obsm\` entries to \`reducedDims\`.
...
- counts_layer:
  ...
  Name(s) of the layer in \`adata\$layers\` to use as the \`counts\`
  assay. May be a single string or a character vector of candidates; the
  first one present in \`adata\$layers\` is used. If none are present,
  \`adata\$X\` is used as \`counts\`. Defaults to \`c("counts",
