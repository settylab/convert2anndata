  ...
  tryCatch(
    {
      ...
      timestamped_cat(sprintf(
        "Successfully extracted %s with internal=TRUE.\n",
        data_type
      ))
    ...
    error = function(e) {
      timestamped_cat(sprintf(
        "WARNING: Failed to extract %s with internal=TRUE.\n",
        data_type
      ))
      tryCatch(
        {
          ...
          timestamped_cat(sprintf(
            "Successfully extracted %s without internal=TRUE.\n",
            data_type
          ))
          missing_columns <- setdiff(internal_columns, colnames(data))
          if (length(missing_columns) > 0) {
            timestamped_cat(
              "WARNING: The following columns are missing without ",
        ...
        error = function(e) {
          ...
          stop(sprintf(
            "Unable to extract %s from the SingleCellExperiment object.",
            data_type
          ), call. = FALSE)
        }
      )
